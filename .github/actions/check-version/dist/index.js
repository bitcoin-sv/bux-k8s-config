...
const current_image_1 = __nccwpck_require__(6025);
const latest_docker_version_tag_1 = __nccwpck_require__(3500);
const gt_1 = __importDefault(__nccwpck_require__(4123));
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        const username = core.getInput('dockerhub_user');
        const password = core.getInput('dockerhub_password');
        const deploymentFile = core.getInput('deployment_file');
        const current = (0, current_image_1.currentImage)(deploymentFile);
        const latestTag = yield (0, latest_docker_version_tag_1.latestDockerVersionTag)({
        ...
        output('hasNewerImage', (0, gt_1.default)(latestTag, current.version));
    });
}
exports.run = run;
...
const currentImage = (path) => {
    ...
    const deployment = yaml_1.default.parse(data.toString());
    const container = (_c = (_b = (_a = deployment === null || deployment === void 0 ? void 0 : deployment.spec) === null || _a === void 0 ? void 0 : _a.template) === null || _b === void 0 ? void 0 : _b.spec) === null || _c === void 0 ? void 0 : _c.containers['0'];
    if (container == null) {
        throw new Error(`Cannot find container specification in a yaml file under path ${path}`);
    }
...
//# sourceMappingURL=current-image.js.map
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.latestDockerVersionTag = void 0;
const httpm = __importStar(__nccwpck_require__(6255));
const am = __importStar(__nccwpck_require__(5526));
const rsort_1 = __importDefault(__nccwpck_require__(8701));
const satisfies_1 = __importDefault(__nccwpck_require__(6055));
const latestDockerVersionTag = ({ username, password, owner, repository }) => __awaiter(void 0, void 0, void 0, function* () {
    if (someIsNull(username, password)) {
        throw new Error('Both username and password must be specified, or none of them');
    }
    const token = username != null && password != null
        ? yield login(username, password)
        : undefined;
    const tags = yield getTags(owner, repository, token);
    const tag = findLatest(tags);
    return tag;
});
exports.latestDockerVersionTag = latestDockerVersionTag;
function login(username, password) {
    return __awaiter(this, void 0, void 0, function* () {
        const http = new httpm.HttpClient();
        const resp = yield http.postJson('https://hub.docker.com/v2/users/login', { username, password });
        if (isSuccess(resp) && isTokenResult(resp)) {
            return resp.result.token;
        }
        else if (isSuccess(resp)) {
            throw new Error('Error when trying to log in to dockerhub. Success result with unexpected body value' +
                JSON.stringify(resp.result));
        }
        else {
            throw new Error('Error when trying to log in to dockerhub.' + JSON.stringify(resp.result));
        }
    });
}
...
function isTokenResult(resp) {
    return (resp.result != null && 'token' in resp.result && resp.result.token != null);
}
function getTags(owner, repository, token = '') {
    return __awaiter(this, void 0, void 0, function* () {
        const http = new httpm.HttpClient('http-client', [
            new am.BearerCredentialHandler(token)
        ]);
        const resp = yield http.getJson(`https://hub.docker.com/v2/repositories/${owner}/${repository}/tags/?page_size=100`);
        if (isSuccess(resp) && isTagsResult(resp)) {
            return resp.result.results;
        }
        else if (isSuccess(resp)) {
            throw new Error('Error when trying to get image tags from dockerhub. Success result with unexpected body value' +
                JSON.stringify(resp.result));
        }
        else {
            throw new Error('Error when trying to get image tags from dockerhub.' +
                JSON.stringify(resp.result));
        }
    });
}
function isTagsResult(resp) {
    return (resp.result != null &&
        'results' in resp.result &&
        resp.result.results != null);
}
function findLatest(tags) {
    const fullVersions = tags.map(t => t.name).filter(t => (0, satisfies_1.default)(t, 'x.x.x'));
    const sorted = (0, rsort_1.default)(fullVersions);
    const latest = sorted.shift();
    if (latest == null) {
        throw new Error('Cannot find latest full version number within tags.' +
            JSON.stringify(tags, null, 1));
    }
    return latest;
}
function someIsNull(...args) {
    const nonNullElements = args.filter(it => it != null);
    return nonNullElements.length !== 0 && nonNullElements.length < args.length;
}
//# sourceMappingURL=latest-docker-version-tag.js.map
